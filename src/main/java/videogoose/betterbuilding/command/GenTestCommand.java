...
public class GenTestCommand implements CommandInterface {
	...
	public String getDescription() {
		return """
				Phase-0 test: build a synthetic asymmetric template in code and load it into PASTE mode.
	...
	public boolean onCommand(PlayerState sender, String[] args) {
		try {
			...
			BetterBuilding.getInstance().logInfo("[BB] gentest wrote " + smtpl.getAbsolutePath() + " (" + t.blockCount() + " blocks)");
			...
			PlayerUtils.sendMessage(sender, "[BB] gentest template loaded into PASTE mode (" + t.blockCount() + " blocks, " + t.dimX() + "x" + t.dimY() + "x" + t.dimZ() + "). " + "Expect: a hull box with a notch cut from the +X/top/front corner and a tall fin " + "on the +X side only. If chirality/size look right, the output path is verified.");
		} catch(Exception e) {
