...
public class LuaExecutor {
	...
	private final VoxelTemplate template;
	private final int[] dims;
	private final Map<String, Short> palette;
	private int totalOps;
	private String templateName;

	public LuaExecutor(VoxelTemplate template, int[] dims, Map<String, Short> palette) {
	...
	/**
	 * Lenient optional-int read: coerces a boolean (true->1, false->0) so the model
	 * can put a hollow flag in an int slot (e.g. cone's tip_r position) without a
	 * hard "int expected, got boolean" crash. Falls back to {@code def} for nil.
	 */
	private static int optInt(Varargs args, int i, int def) {
		if (args.narg() < i) return def;
		LuaValue v = args.arg(i);
		if (v.isnil()) return def;
		if (v.isboolean()) return v.toboolean() ? 1 : 0;
		return v.optint(def);
	}

	/** Lenient optional-boolean read: coerces a number (nonzero -> true) so a number in a flag slot won't crash. */
	private static boolean optBool(Varargs args, int i, boolean def) {
		if (args.narg() < i) return def;
		LuaValue v = args.arg(i);
		if (v.isnil()) return def;
		if (v.isnumber()) return v.toint() != 0;
		return v.optboolean(def);
	}
	...
	private void registerPrimitives(Globals globals) {
		...
		globals.set("fill", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				byte orient = args.narg() >= 8 ? (byte) optInt(args, 8, 0) : 0;
		...
		globals.set("shell", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				int tx = args.checkint(4), ty = args.checkint(5), tz = args.checkint(6);
				short type = (short) args.checkint(7);
				byte orient = args.narg() >= 9 ? (byte) optInt(args, 9, 0) : 0;
				int thickness = args.narg() >= 8 ? optInt(args, 8, 1) : 1;
		...
		globals.set("place", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				byte orient = args.narg() >= 5 ? (byte) optInt(args, 5, 0) : 0;
		...
		globals.set("ellipsoid", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				short type = (short) args.checkint(7);
				byte orient = args.narg() >= 9 ? (byte) optInt(args, 9, 0) : 0;
				boolean hollow = args.narg() >= 8 && optBool(args, 8, false);
		...
		globals.set("cylinder", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				byte orient = args.narg() >= 10 ? (byte) optInt(args, 10, 0) : 0;
				boolean hollow = args.narg() >= 9 && optBool(args, 9, false);
		...
		globals.set("cone", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				short type = (short) args.checkint(8);
				int tipRadius = args.narg() >= 9 ? optInt(args, 9, 0) : 0;
				byte orient = args.narg() >= 11 ? (byte) optInt(args, 11, 0) : 0;
				boolean hollow = args.narg() >= 10 && optBool(args, 10, false);
		...
		globals.set("arc", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				int thickness = args.narg() >= 9 ? optInt(args, 9, 1) : 1;
				byte orient = args.narg() >= 10 ? (byte) optInt(args, 10, 0) : 0;
		...
		globals.set("replace", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				byte newOrient = args.narg() >= 9 ? (byte) optInt(args, 9, -1) : -1;
		...
		globals.set("count_blocks", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				int fx = args.checkint(1), fy = args.checkint(2), fz = args.checkint(3);
				int tx = args.checkint(4), ty = args.checkint(5), tz = args.checkint(6);
				int filterType = args.narg() >= 7 ? optInt(args, 7, -1) : -1;
		...
		globals.set("noise", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				long seed = args.narg() >= 9 ? (long) optInt(args, 9, 0) : 0;
				byte orient = args.narg() >= 10 ? (byte) optInt(args, 10, 0) : 0;
		...
		globals.set("hollow", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				int fx = args.checkint(1), fy = args.checkint(2), fz = args.checkint(3);
				int tx = args.checkint(4), ty = args.checkint(5), tz = args.checkint(6);
				int thickness = args.narg() >= 7 ? optInt(args, 7, 1) : 1;
		...
		globals.set("extrude", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				short type = (short) args.checkint(7);
				byte orient = args.narg() >= 8 ? (byte) optInt(args, 8, 0) : 0;
				boolean copyExisting = args.narg() >= 9 && optBool(args, 9, false);
		...
		globals.set("flood_fill", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				short type = (short) args.checkint(4);
				byte orient = args.narg() >= 5 ? (byte) optInt(args, 5, 0) : 0;
				int maxBlocks = args.narg() >= 6 ? optInt(args, 6, 100000) : 100000;
		...
		globals.set("smooth", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				int fx = args.checkint(1), fy = args.checkint(2), fz = args.checkint(3);
				int tx = args.checkint(4), ty = args.checkint(5), tz = args.checkint(6);
				int iterations = args.narg() >= 7 ? optInt(args, 7, 1) : 1;
		...
		globals.set("dilate", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				int tx = args.checkint(4), ty = args.checkint(5), tz = args.checkint(6);
				short type = (short) args.checkint(7);
				int iterations = args.narg() >= 8 ? optInt(args, 8, 1) : 1;
				byte orient = args.narg() >= 9 ? (byte) optInt(args, 9, 0) : 0;
		...
		globals.set("mirror", new VarArgFunction() {
			public Varargs invoke(Varargs args) {
				...
				int minX = args.narg() >= 2 ? optInt(args, 2, 0) : 0;
				int minY = args.narg() >= 3 ? optInt(args, 3, 0) : 0;
				int minZ = args.narg() >= 4 ? optInt(args, 4, 0) : 0;
				int maxX = args.narg() >= 5 ? optInt(args, 5, dims[0] - 1) : dims[0] - 1;
				int maxY = args.narg() >= 6 ? optInt(args, 6, dims[1] - 1) : dims[1] - 1;
				int maxZ = args.narg() >= 7 ? optInt(args, 7, dims[2] - 1) : dims[2] - 1;
