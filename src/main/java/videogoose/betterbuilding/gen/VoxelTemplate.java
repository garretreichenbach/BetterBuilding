...
public final class VoxelTemplate {
	...
	public VoxelTemplate(String name, int dx, int dy, int dz) {
		...
		this.dz = dz;
		int n = dx * dy * dz;
		this.types = new short[n];
		this.orients = new byte[n];
	}

	// --- dimensions / identity ---

	public int dimX() { return dx; }
	public int dimY() { return dy; }
	public int dimZ() { return dz; }
	...
	public static VoxelTemplate fromCopyArea(String name, CopyArea area) {
		...
		if (!pieces.isEmpty()) {
			...
			for (int i = 0; i < pieces.size(); i++) {
				...
				pMinX = Math.min(pMinX, p.voidPos.x); pMaxX = Math.max(pMaxX, p.voidPos.x);
				pMinY = Math.min(pMinY, p.voidPos.y); pMaxY = Math.max(pMaxY, p.voidPos.y);
				pMinZ = Math.min(pMinZ, p.voidPos.z); pMaxZ = Math.max(pMaxZ, p.voidPos.z);
			}
			if (!storedValid
